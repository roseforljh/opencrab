...
const Playground = () => {
  ...
  return (
    <PlaygroundProvider value={playgroundContextValue}>
      <div className='h-full bg-semi-color-bg-0'>
        <div className='border-b border-semi-color-border bg-semi-color-bg-1 px-4 py-4 md:px-6'>
          <div className='flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between'>
            <div>
              ...
              <p className='mt-1 text-sm text-gray-500 dark:text-gray-400'>
                {t(
                  '这里是你的模型调试台：左侧配置请求，中间编排消息，右侧查看调试结果。',
                )}
              </p>
            </div>
          </div>
        ...
        <Layout className='h-full bg-transparent flex flex-col md:flex-row'>
          {(showSettings || !isMobile) && (
            <Layout.Sider
              className={`
              bg-semi-color-bg-1 border-r border-semi-color-border flex-shrink-0 overflow-auto
              ${
                isMobile
          ...
          <Layout.Content className='relative flex-1 overflow-hidden'>
            <div className='flex h-[calc(100vh-130px)] flex-col overflow-hidden lg:flex-row'>
              <div className='flex min-w-0 flex-1 flex-col border-r border-semi-color-border bg-semi-color-bg-0'>
                <ChatArea
                  chatRef={chatRef}
                  message={message}
                  inputs={inputs}
              ...
              {showDebugPanel && !isMobile && (
                <div className='w-[380px] flex-shrink-0 h-full bg-semi-color-bg-1'>
                  <OptimizedDebugPanel
                    debugData={debugData}
            ...
            {showDebugPanel && isMobile && (
              <div className='fixed top-0 left-0 right-0 bottom-0 z-[1000] bg-semi-color-bg-1 overflow-auto shadow-lg'>
                <OptimizedDebugPanel
                  debugData={debugData}
                  activeDebugTab={activeDebugTab}
                  onActiveDebugTabChange={setActiveDebugTab}
