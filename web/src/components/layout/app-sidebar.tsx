...
export function AppSidebar() {
  ...
  useEffect(() => {
    const linksToPrefetch = navigationItems
      .map((item) => item.href)
    ...
    const timeoutId = globalThis.setTimeout(prefetchAll, 150);
  ...
  return (
    <aside
      className={`hidden h-screen shrink-0 border-r border-border bg-background transition-[width] duration-300 ease-out lg:flex lg:flex-col ${
        collapsed ? "w-20" : "w-72"
      }`}
    >
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-4 py-6">
        ...
        <nav className="space-y-1">
          {navigationItems.map((item) => {
            ...
            const switching = isPending && pendingHref === item.href;
            ...
            return (
              <button
                ...
                className={`group relative flex w-full items-center overflow-hidden rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                  active
              ...
              >
                ...
                <div
                  className={`relative z-[1] flex min-w-0 flex-1 items-center justify-between overflow-hidden whitespace-nowrap transition-[max-width,opacity,transform] duration-300 ease-out ${
