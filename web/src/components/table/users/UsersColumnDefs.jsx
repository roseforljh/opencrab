...
const renderUsername = (text, record) => {
  ...
  if (!remark) {
    return <span className='text-white/88'>{text}</span>;
  }
  const maxLen = 10;
  const displayRemark =
  ...
  return (
    <Space spacing={2}>
      ...
      <Tooltip content={remark} position='top' showArrow>
        <Tag color='white' shape='circle' className='!border !border-white/10 !bg-white/6 !text-xs !text-white'>
...
const renderStatistics = (text, record, showEnableDisableModal, t) => {
  ...
  const tooltipContent = (
    <div className='text-xs'>
      <div>
        {t('调用频次')}: {renderNumber(record.request_count)}
      </div>
    </div>
  );
