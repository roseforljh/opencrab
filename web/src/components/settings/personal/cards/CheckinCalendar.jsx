  ...
  return (
    <Card className='!rounded-[28px] !border !border-white/10 !bg-white/6 !shadow-[0_24px_80px_rgba(0,0,0,0.28)] !backdrop-blur-xl'>
      ...
      <div className='flex items-center justify-between'>
        ...
        >
          <Avatar size='small' color='grey' className='mr-3 !bg-white/80 !text-black shadow-[0_10px_24px_rgba(0,0,0,0.15)]'>
      ...
      <Collapsible isOpen={isCollapsed === false} keepDOM>
        ...
        <div className='mb-4 mt-4 grid grid-cols-3 gap-3'>
          <div className='rounded-[20px] border border-white/10 bg-[#0d1527]/80 p-2.5 text-center'>
            <div className='text-xl font-bold text-[#7af0d2]'>
              {checkinData.stats?.total_checkins || 0}
            </div>
            <div className='text-xs text-white/45'>{t('累计签到')}</div>
          </div>
          <div className='rounded-[20px] border border-white/10 bg-[#0d1527]/80 p-2.5 text-center'>
            <div className='text-xl font-bold text-[#ffcf5a]'>
              {renderQuota(monthlyQuota, 6)}
            </div>
            <div className='text-xs text-white/45'>{t('本月获得')}</div>
          </div>
          <div className='rounded-[20px] border border-white/10 bg-[#0d1527]/80 p-2.5 text-center'>
            <div className='text-xl font-bold text-[#7cc7ff]'>
              {renderQuota(checkinData.stats?.total_quota || 0, 6)}
            </div>
            <div className='text-xs text-white/45'>{t('累计获得')}</div>
          </div>
        </div>

        {/* 签到日历 - 使用更紧凑的样式 */}
        <Spin spinning={loading}>
          <div className='checkin-calendar overflow-hidden rounded-[20px] border border-white/10 bg-[#0d1527]/80'>
            <style>{`
        ...
        {/* 签到说明 */}
        <div className='mt-3 rounded-[20px] border border-white/10 bg-[#0d1527]/80 p-2.5'>
          <Typography.Text className='text-xs !text-white/52'>
