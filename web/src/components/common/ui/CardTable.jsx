  ...
  if (showSkeleton) {
    ...
    const renderSkeletonCard = (key) => {
        ...
        <div className='p-2'>
          {visibleCols.map((col, idx) => {
            ...
            return (
              <div
                key={idx}
                className='flex items-center justify-between border-b border-white/8 py-1 last:border-b-0'
              >
      ...
      return (
        <Card
          key={key}
          className='!rounded-[24px] !border !border-white/10 !bg-white/6 !shadow-[0_24px_60px_rgba(0,0,0,0.24)] !backdrop-blur-xl'
        >
      ...
      >
        {columns.map((col, colIdx) => {
          ...
          return (
            <div
              key={col.key || colIdx}
              className='flex items-start justify-between border-b border-white/8 py-1 last:border-b-0'
            >
              <span className='mr-2 select-none whitespace-nowrap font-medium text-white/45'>
                {title}
              </span>
              <div className='flex flex-1 items-center justify-end gap-1 break-all text-white/88'>
        ...
        {hasDetails && (
          <>
            <Button
              theme='borderless'
              size='small'
              className='!mt-3 !flex !w-full !justify-center !rounded-2xl !border !border-white/10 !bg-white/6 !text-white hover:!bg-white/10'
