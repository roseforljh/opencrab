...
const CardPro = ({
  ...
  const renderFooter = () => {
    ...
    return (
      <div
        className={`flex w-full border-t border-white/10 pt-4 ${isMobile ? 'justify-center' : 'items-center justify-between'}`}
      >
        {paginationArea}
      </div>
    );
  ...
  return (
    <Card
      className={`table-scroll-card !overflow-hidden !rounded-[28px] !border !border-white/10 !bg-white/6 !shadow-[0_30px_100px_rgba(0,0,0,0.32)] !backdrop-blur-xl ${className}`}
      title={headerContent}
