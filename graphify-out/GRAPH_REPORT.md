...
## Corpus Check
- 160 files ¡¤ ~149,440 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1598 nodes ¡¤ 2687 edges ¡¤ 77 communities detected
- Extraction: 100% EXTRACTED ¡¤ 0% INFERRED ¡¤ 0% AMBIGUOUS
- Token cost: 0 input ¡¤ 0 output

## God Nodes (most connected - your core abstractions)
1. `roundTripFunc` - 51 edges
2. `testGatewayMessage()` - 49 edges
3. `testGatewayMessage()` - 29 edges
4. `newGatewayServiceForTest()` - 27 edges
5. `decodeJSONObject()` - 25 edges
6. `GetDashboardSummary()` - 18 edges
7. `sanitizeRequestMetadataForTarget()` - 17 edges
8. `minimalOpenAIChatResponseBody()` - 16 edges
9. `writeGatewayResult()` - 16 edges
10. `renderGatewayErrorForProtocol()` - 16 edges
...
Cohesion: 0.04
Nodes (92): applyRequestHeaders(), asInt(), attachPayloadDebugMetadata(), buildExecutorPayload(), buildFocusedPayloadPreview(), buildToolNameReverseMap(), claudeBudgetToOpenAIEffort(), claudeToolPartToGeminiFunctionResponse() (+84 more)
...
Cohesion: 0.03
Nodes (38): testHTTPClient(), TestKimiRealChannel(), TestOpenRouterRealChannel(), buildGeminiCachedContentResponse(), decodeGeminiCachedContentName(), HandleGeminiCachedContentCreate(), HandleGeminiCachedContentGet(), mustMarshalJSON() (+30 more)
...
Cohesion: 0.09
Nodes (56): gjsonToolCalls(), minimalClaudeResponseBody(), minimalGeminiResponseBody(), minimalOpenAIChatResponseBody(), TestClaudeExecutorBuildsMultimodalRequest(), TestClaudeExecutorBuildsNativeTextRequest(), TestClaudeExecutorDoesNotForwardAuthorizationHeaderFromClientRequest(), TestClaudeExecutorPassesThroughAnthropicBetaHeader() (+48 more)
...
Cohesion: 0.08
Nodes (43): newGatewayServiceForTest(), stickyKey(), testGatewayMessage(), TestGatewayServiceAllAttemptsFailedAfterThreeRetriesPerRoute(), TestGatewayServiceAllowsClaudeComputerUseBridgeWhenNoClaudeRouteExists(), TestGatewayServiceAllowsClaudeContextManagementBridgeWhenNoClaudeRouteExists(), TestGatewayServiceAllowsClaudePromptCachingBridgeWhenNoClaudeRouteExists(), TestGatewayServiceAnnotatesSuccessfulResultWithTargetOperation() (+35 more)
...
Cohesion: 0.12
Nodes (20): adaptRequestForProvider(), buildRoutingCursorKey(), groupRoutesByPriority(), invocationBucketName(), marshalGatewayRequest(), nativePreferredProvider(), preferNativeProviderRoutes(), prioritizeRoutesByInvocationMode() (+12 more)

### Community 13 - "Community 13"
Cohesion: 0.12
Nodes (31): buildClaudeSource(), buildClaudeToolResultBlocks(), claudeThinkingSupported(), claudeToolChoiceDisallowsThinking(), claudeTopLevelAndNestedCacheTTL(), collectClaudeCacheControlTTLs(), collectClaudeCacheControlTTLsInto(), containsCacheTTL() (+23 more)
...
Cohesion: 0.25
Nodes (22): collectUnknownFields(), DecodeOpenAIChatRequest(), DecodeOpenAIChatResponse(), DecodeOpenAIChatStream(), decodeOpenAIMessageContent(), decodeOpenAIPart(), decodeOpenAIResponseMessage(), decodeOpenAIToolCalls() (+14 more)
...
Cohesion: 0.31
Nodes (7): EncodeClaudeChatStream(), EncodeOpenAIChatStream(), firstUsageValue(), mustClaudeSSE(), mustOpenAISSE(), orderedResponseItems(), orderedResponseItem
...
Cohesion: 0.52
Nodes (5): rawMapHasKeys(), requestRequiresProtocolMatchedRoute(), requestUsesClaudeNativeFeatures(), requestUsesGeminiNativeFeatures(), requestUsesOpenAINativeSession()
...
## Knowledge Gaps
- **128 isolated node(s):** `surfaceSupport`, `providerMatrix`, `ScopeType`, `ProfileRecord`, `Loader` (+123 more)
  These have ¡Ü1 connection - possible missing edges or undocumented components.
- **Thin community `Community 65`** (2 nodes): `capability_store_test.go`, `TestCapabilityProfileStoreListCapabilityProfiles()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 66`** (2 nodes): `db.go`, `Open()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 67`** (2 nodes): `migrate_test.go`, `TestApplyMigrations()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 68`** (2 nodes): `response_session_store_test.go`, `TestResponseSessionStoreRoundTrip()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 69`** (2 nodes): `not-found.tsx`, `NotFound()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 70`** (2 nodes): `loading.tsx`, `ConsoleLoading()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 71`** (2 nodes): `template.tsx`, `ConsoleTemplate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 72`** (2 nodes): `loading-state.tsx`, `LoadingState()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (2 nodes): `admin-api-server.test.ts`, `assert()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 74`** (1 nodes): `next-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 75`** (1 nodes): `tailwind.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 76`** (1 nodes): `console-data.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
