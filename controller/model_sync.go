...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/opencrab/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/opencrab/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/opencrab/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/opencrab/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/opencrab/models.json", base), fmt.Sprintf("%s/api/opencrab/vendors.json", base)
}
