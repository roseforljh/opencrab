...
type User struct {
	...
	PinHash          string         `json:"-" gorm:"column:pin_hash;type:varchar(255)"`
...
func (user *User) SetPin(pin string) error {
	hashedPin, err := common.Password2Hash(pin)
	if err != nil {
		return err
	}
...
func (user *User) ValidatePin(pin string) bool {
	if pin == "" {
		return false
	}
	if user.PinHash != "" {
		return common.ValidatePasswordAndHash(pin, user.PinHash)
	}
	return common.ValidatePasswordAndHash(pin, user.Password)
}
